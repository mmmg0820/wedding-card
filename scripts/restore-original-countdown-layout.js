...
const compactCountdownCss = `  #events .countdown{
    width:min(100%,286px);
...
`;
html = replaceOnce(html, compactCountdownCss, '', 'remove compact countdown override');
html = replaceOnce(html, '/* ---------- compact countdown and lightbox gestures ---------- */', '/* ---------- lightbox gestures ---------- */', 'lightbox comment');

if (html.includes('width:min(100%,286px)') || html.includes('min-height:76px')) {
  throw new Error('compact countdown override remains');
}
if (!html.includes('addEventListener("touchend"') || !html.includes('addEventListener("dblclick"')) {
  throw new Error('gallery lightbox gestures are missing');
}
...
console.log('Restored the countdown layout used before the 64px card adjustment.');
