...
const sizeCss = `
  ...
  /* ---------- gallery lightbox 80% viewport limit ---------- */
...
`;
html = replaceOnce(html, '</style></head>', `${sizeCss}</style></head>`, 'style closing tag');

if (!html.includes('max-width:80vw;') || !html.includes('max-height:80dvh;')) {
  throw new Error('80% lightbox limit missing');
}
if (!html.includes('event.target === galleryLightbox')) {
  throw new Error('background close behavior is missing');
}
...
console.log('Limited enlarged gallery photos to 80% of the visible viewport.');
